...
The source was compiled in Borland Turbo C++ 3.0 in PCDOS 7 running inside 86Box 4.2 (because it supports emulating the XGA and IBM PS/2 hardware). There is only one source file (xgainfo.c) and relies only on the C standard library TC++ 3.0 supplies. Just open the file in the IDE and compile to run it.
